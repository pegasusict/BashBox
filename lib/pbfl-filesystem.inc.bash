#!/bin/bash
################################################################################
## Pegasus' Linux Administration Tools #                Bash Function Library ##
## (C)2017-2020 Mattijs Snepvangers    #                pegasus.ict@gmail.com ##
## License: MIT                        #   Please keep my name in the credits ##
################################################################################
## SCRIPT_TITLE="FileSystem Functions"
## VERSION=( 0 0 0 "DEV" 20200701 )
################################################################################

### FileSystem operations ####################################################
add_line_to_file() { ### Inserts line into file if it's not there yet
  local _LINE_TO_ADD=$1
  local _TARGET=$2
  dbg_line "LINE_TO_ADD: $_LINE_TO_ADD"
  dbg_line "TARGET: $_TARGET"
  if [ $(line_exists ${_LINE_TO_ADD} ${_TARGET}) ]
  then
    info_line "${FUNCNAME[0]}: line already exists, skipping..."
  else
    if [ -w "$_TARGET" ]
    then
